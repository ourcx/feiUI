<template>
  <Teleport to="body">
    ...
    <transition :name="slideTransitionName" @after-leave="handleAfterLeave">
      <div class="fei-drawer-wrapper" :style="wrapperStyle" v-if="visible">
        <header class="fei-drawer-header" v-if="showClose || title">
          ...
          <button
            v-if="showClose"
            @click="handleClose"
            class="fei-drawer-close-btn"
            :class="{
              'fei-drawer-close-btn--disabled': !maskClosable,
              ['fei-drawer-close-btn-' + type]: type,
            }"
          >
            &times;
          </button>
